...
The implementation of the `order` command is as shown : //insert code +

As you can see from the implementation, the command requires user inputs in the form of 'String' as the paramater/s
to order. The inputs are restricted by the fields of `Person` which includes `Name`, `Address`,
...
The user executes an `order` command by typing `order name`, to view the list of contacts ordered in alphabetical order
...
=== Group mechanism - Coming in V1.4

The group mechanism will be facilitated by the `Person', which resides inside 'Addressbook'. It supports
grouping contacts into user defined group by utilizing the CLI commands in the TunedIn application.
