...
Finds persons by names or addresses that contain any of the given keywords. +
Format: `find <attribute>/KEYWORD [MORE_KEYWORDS]` or `f <attribute>/KEYWORD [MORE_KEYWORDS]`

****
* `<attribute>` refers to either `n`, standing for name, or `a`, standing for address. This is case-sensitive.
