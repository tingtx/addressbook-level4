...
public class LockCommandParser implements Parser<LockCommand> {
    ...
    public LockCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_USERID, PREFIX_PASSWORD);

        if (!arePrefixesPresent(argMultimap, PREFIX_USERID, PREFIX_PASSWORD)) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, LockCommand.MESSAGE_USAGE));
        }

        String userId = argMultimap.getValue(PREFIX_USERID).get();
        String password = argMultimap.getValue(PREFIX_PASSWORD).get();

        return new LockCommand(userId, password);
    }
}
